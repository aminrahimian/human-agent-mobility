...
from main_module import main_novideo, main_video, main_timeana
...
def draw_novideo(targets, beta, agents, mu, alpha, rho, Rv, runs, save):
    trees = [targets]
    betas = [beta]
    agents = [agents]
    mu_range = [mu]
    ...
    while z < runs:
        ...
        print(paramlist[0])
        result = main_novideo(paramlist[0])
        if save:
            f2.write(result)
        z += 1
...
def draw_video(targets, beta, agents, mu, alpha, rho, Rv, tick_speed, videofilename, runs, save):
    ...
    while z < runs:
        ...
        result = main_video(paramlist[0], fig, ax, tick_speed, videofilename)
        if save:
            f2.write(result)
        z += 1
...
def timedata(targets, beta, agents, mu, alpha, rho, Rv, tick_speed, runs, hdf5filename, CaseCount ,save):
    trees = [targets]
    betas = [beta]
    agents = [agents]
    mu_range = [mu]
    ...
    while z < runs:
        ...
        print(paramlist[0])
        result = main_novideo(paramlist[0])
        caseid = z + CaseCount # numbwe of existed groups
        main_timeana(paramlist[0], tick_speed, hdf5_filename=hdf5filename, case_id=caseid)
        if save:
            f2.write(result)
        z += 1
...
def draw_video_timedata(targets, beta, agents, mu, alpha, rho, Rv, tick_speed, runs, videofilename, hdf5filename, save):
    fig = plt.figure(figsize=(12, 12))
    ax = fig.subplots(1, 1)
