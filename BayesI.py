...
#     likelihood_ones=[-(distance.euclidean((set_observations[i,0],set_observations[i,1] ), (theta1,theta2)))*(1/50) for i in range(int(set_observations.shape[0])) if set_observations[i,2]==1]    
#     likelihood_zeros=[ np.log(1-np.exp(-(distance.euclidean((set_observations[i,0],set_observations[i,1] ), (theta1,theta2)))*(1/50))) for i in range(int(set_observations.shape[0])) if set_observations[i,2]==0]
   
  
    
#     return sum(likelihood_ones)+ sum(likelihood_zeros)+ np.log(f_theta1) +np.log(f_theta2)
...
def choosing_direction(nn,L,H,t1,t2):
    ...
    # file_posterior=str(int(kk))+'\\input'+ '_'+str(int(nn))+'.csv'
    ...
    local_opt=[eval_coordinates.iat[np.argmax(col2),0][0], eval_coordinates.iat[np.argmax(col2),0][1]]
    
    
    # bounds=np.array([[0,2000],[0,2000]])

    # xp, yp= gp.bayesian_optimisation(n_iters=30, sample_loss=evaluation_theta(set_observations,prior_theta1,prior_theta2, sd_prior),
    #                                  bounds=bounds,
    #                                  n_pre_samples=10)


    # local_opt=xp[np.array(yp).argmax(),:]
