...
%     disp(num2str(idx1));
    ...
    Real_Target = [Real_Target_All(idx1,1) * Dim1, Real_Target_All(idx1,2) * Dim2];
    ...
    output_mat = [];
    output_mat = [ones(N_sample,2).*xy_pede, binornd(1,Likely,[N_sample,1])];

    % initial time index
    t = 1;

    % write output matrix to .csv file
    if ~exist(num2str(idx1),'dir')
        mkdir(num2str(idx1));
    end
    writematrix(output_mat,[num2str(idx1),'\input_',num2str(t),'.csv']);
...
%         output_mat(t,3) = binornd(1,Likely);
        temp_a = [ones(N_sample,2).*xy_pede, binornd(1,Likely,[N_sample,1])];
        output_mat = [output_mat;temp_a];
        ...
        delete([num2str(idx1),'\input_*.csv']);
    ...
    TimeLength_record(idx1) = T_total/3600;
    Regret_record{idx1} = Regret;
    disp(['Realization ',num2str(idx1),' using ',num2str(T_total/3600),' hours']);

    rmdir(num2str(idx1),"s");
end
