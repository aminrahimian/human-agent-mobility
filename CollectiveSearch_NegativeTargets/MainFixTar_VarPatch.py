# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 12:43:20 2024
...
class Grid:
    ...
    def __init__(self, radiuss, tc, agents, beta, l, a, rho, Rv, num_patches):
    ...
    def crea_targets(self, num_patches, target_location = None):
        self.size = 1
        ...
        if target_location:
            for i, (x, y) in enumerate(target_location):
...
def main_timeana(params, tick_speed, hdf5_filename, case_id, num_patches, target_locations = None):
    radiuss = params[3]
    tc = params[0]
    mono = params[1]
    beta = params[2]
    ...
    grid = Grid(radiuss, tc, mono, beta, l, a, rho, Rv)
    grid.crea_targets(num_patches, target_locations)
...
def main_video_timedata(params,fig, ax, tick_speed, video_filename, hdf5_filename, case_id, num_patches):    
#     from Grid import Grid
    radiuss = params[3]
    tc = params[0]
    mono = params[1]
    ...
    grid = Grid(radiuss, tc, mono, beta, l, a, rho, Rv, num_patches)
    i = 1
    # Main cycle of each iteration
    all_rep = ""
    mainn = ""
...
def target_structure(params, num_patches):
#     from Grid import Grid
    radiuss = params[3]
    tc = params[0]
    mono = params[1]
