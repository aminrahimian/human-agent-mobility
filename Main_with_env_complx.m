...
dT_command = 60; % time for movement after velocity adjustment
dT = 2;
R = 100; % detection radius parameter
radius_factor_best = 2; % The radius factor of receiving signal under best env condition

% agent response to boundary
phi = 100/180*pi; % angle of field of view
c_wall = 1;
...
% generate trees according to poisson distribution
NumberOfTrees = 50000;
Tree_center = [rand(NumberOfTrees,1)*Dim1,...
               rand(NumberOfTrees,1)*Dim2];
Tree_diameter = rand(NumberOfTrees,1) * 0.5;
...
%         disp([num2str(T_total/3600),' hours']);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if rem(T_total,dT_command) == 0
            idx = pyrunfile('Bayesian_posterior.py', 'theta',...
                nn = t, L = Dim1, H = Dim2);
...
%         [F_alpha_B_1]=calc_F_alpha_B(xy_pede,r_target,phi,c_wall,Boundary_effective);
        [F_alpha_B_2]=calc_F_alpha_B_point(xy_pede,r_target,phi,c_wall,Tree_center,Tree_diameter);
        
        F_tot = F_Di + F_alpha_B_2;

        uv_pede_old = uv_pede; % save the previous step velocity
        uv_pede = uv_pede + F_tot*dt; % update velocity
...
figure(2);
hold on
plot(Tree_center(:,1),Tree_center(:,2),'g.','MarkerSize',0.1);
